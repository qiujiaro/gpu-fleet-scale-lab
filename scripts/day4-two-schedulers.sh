#!/usr/bin/env bash
# Day 4 experiment (guide §8): submit the same workload twice, half to default-scheduler
...
DUR="${DUR:-60}"
QPS="${QPS:-10}"
OUT_DIR="${OUT_DIR:-experiments/_raw}"
mkdir -p "$OUT_DIR"
...
go run ./cmd/loadgen --arrival constant --qps "$QPS" --duration "$DUR" \
  ...
  --out "$OUT_DIR/day4-default.jsonl" &
pid_default=$!
go run ./cmd/loadgen --arrival constant --qps "$QPS" --duration "$DUR" \
  --scheduler-name topogang --seed 43 \
  --out "$OUT_DIR/day4-topogang.jsonl" &
pid_topo=$!
