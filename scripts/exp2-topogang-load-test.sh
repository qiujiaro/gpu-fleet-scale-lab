...
BATCH_ID="${BATCH_ID:-exp2-load-$(date +%Y%m%d-%H%M%S)-$$}"
RESULT_ROOT="${RESULT_ROOT:-experiments/exp2-topogang/controlled-load/local/${BATCH_ID}}"
...
{
  echo "== Exp2 controlled-load matrix =="
...
for qps in ${QPS_LEVELS}; do
  ...
  for repeat in $(seq 1 "${REPEATS}"); do
    ...
    RUN_ID="${run_id}" \
      ...
      ./scripts/exp2-topogang-smoke.sh 2>&1 | tee "${run_dir}.console.log"
    run_rc="${PIPESTATUS[0]}"
    set -e
