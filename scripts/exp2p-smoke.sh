...
#   CLEANUP_OLD_PODS=false CLEANUP_PODS=false ./scripts/exp2p-smoke.sh
set -euo pipefail

cd "$(dirname "$0")/.."

RUN_ID="${RUN_ID:-exp2p-smoke-$(date +%Y%m%d-%H%M%S)-$$}"
...
RESULT_DIR="${RESULT_DIR:-experiments/exp2p-gang-profile/${RUN_ID}}"
...
mkdir -p "${RESULT_DIR}"
tmp_dir="$(mktemp -d /tmp/exp2p-smoke.XXXXXX)"
...
"${tmp_dir}/profiler" \
  --namespace "${NAMESPACE}" \
  --label-selector "exp2p.dev/run-id=${RUN_ID}" \
