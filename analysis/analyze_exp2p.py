#!/usr/bin/env python3
"""Analyze an Exp2-P stepped-load batch.

Input is the summary.csv written by scripts/exp2p-load-test.sh. When the
...
Example:
    ...
    python3 analysis/analyze_exp2p.py \
      --input experiments/exp2p-gang-profile/exp2p-load-.../summary.csv
...
def write_report(
    ...
    lines = [
        f"# Exp2-P performance report — `{batch_id}`",
        "",
...
def default_output(summary_path: Path, runs: list[dict]) -> Path:
    batch_id = runs[0].get("batch_id") or summary_path.parent.name
    return Path("analysis") / "exp2p" / str(batch_id)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Analyze an Exp2-P load-test summary and generate plots/report."
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    parser.add_argument(
        "--out",
        type=Path,
        help="Output directory (default: analysis/exp2p/<batch-id>).",
    )
