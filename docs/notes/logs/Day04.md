# Day 4 — Exp2 phase A: second-scheduler isolation scaffold
...
| `scripts/exp2-two-schedulers.sh` | done (vibe) | the §8 split-workload experiment + the kubectl assertions |
...
- [ ] E2. `scripts/exp2-two-schedulers.sh` — same workload, half `default-scheduler`, half `topogang`, submitted concurrently.
...
go run ./cmd/loadgen --arrival constant --qps 10 --duration 60 \
  --scheduler-name topogang --out experiments/exp2-topogang/two-scheduler-isolation/topogang.jsonl
