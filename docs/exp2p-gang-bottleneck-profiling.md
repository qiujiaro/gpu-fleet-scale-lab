# Exp2-P — Where is the TopoGang scheduler's bottleneck?

**Status: design only. No number in this document is measured.** Sections marked
_Results_ stay empty until the runs in §7 are executed; the design is fixed first so the
findings cannot be reverse-engineered into a nicer story (same rule as
...
| B5 | the N×ρ effect on `t_prefilter`, `t_requeue`, and `t_group_ready` could not be tested | pre-fill to a target ρ and report achieved ρ from bound filler Pods | **Implemented; cluster validation pending** in [scripts/prefill-gpu.sh](../scripts/prefill-gpu.sh) |
...
8. Drain 60 s. Stop. Write `<run>-meta.json`, per-pod CSV, per-group CSV, metrics
   snapshots, and pprof files under `experiments/exp2p-gang-profile/`.
...
- `exp2p-extension-point-breakdown.png` — stacked per-extension-point P99 vs N, topogang
  vs control arm. The main result.
- `exp2p-time-decomposition.png` — the §1 identity as stacked bars per cell.
- `exp2p-prefilter-scan-vs-rho.png` — `nodes_scanned` distribution across ρ (H1).
- `exp2p-cpu-per-admitted-group.png` — efficiency vs N and M.
- `exp2p-queue-timeline.png` — `pending_pods` by queue over time (H4 sawtooth).
